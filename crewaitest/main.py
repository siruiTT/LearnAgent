# 这段代码是用于执行CrewAI项目的主文件。它的功能是提供一种接口来启动项目的各个task和agent
...
'''
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "sk-mQnOyBvhUF3BqfHv2IUogEGu3QS1oPr2tKmmeEsmDz3tWMnD",
...
# run函数是启动Crew执行的入口
# kickoff是Crew执行的核心方法，它会启动所有agent并运行指定task
def run(topic):
    inputs = {
        "topic": topic
    }
    result = CrewtestprojectCrew().crew().kickoff(inputs=inputs)
    return result
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        #     os.environ["OPENAI_MODEL_NAME"] = ONEAPI_CHAT_MODEL
        #     os.environ["OPENAI_API_KEY"] = ONEAPI_CHAT_API_KEY
        # elif MODEL_TYPE == "ollama":
        #     os.environ["OPENAI_API_BASE"] = OLLAMA_API_BASE
        #     os.environ["OPENAI_MODEL_NAME"] = OLLAMA_CHAT_MODEL
        #     os.environ["OPENAI_API_KEY"] = OLLAMA_CHAT_API_KEY
        # else:
        #     os.environ["OPENAI_API_BASE"] = OPENAI_API_BASE
        #     os.environ["OPENAI_MODEL_NAME"] = OPENAI_CHAT_MODEL
        #     os.environ["OPENAI_API_KEY"] = OPENAI_CHAT_API_KEY
        print("初始化完成")
...
async def chat_completions(request: ChatCompletionRequest):
    try:
        ...
        print(f"格式化的搜索结果: {formatted_response}")
